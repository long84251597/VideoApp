...
public class ProxyCacheManager implements ICacheManager, CacheListener {
    ...
    public static int DEFAULT_MAX_COUNT = 3;
    ...
    public void doCacheLogic(Context context, IMediaPlayer mediaPlayer, String originUrl, Map<String, String> header, File cachePath) {
        ...
        if ((url.startsWith("http") && !url.contains("127.0.0.1")) || url.startsWith("concat://")) {
        ...
        try {
            if (mCacheFile)
                onCacheAvailable(cachePath, url, 100);
    ...
    public String getFilePath(String url, Context context){
        HttpProxyCacheServer proxy = getProxy(context.getApplicationContext());
        if (proxy == null)
            return "";
        else
            return proxy.getCacheFile(url).getAbsolutePath();
    }
    ...
    public static HttpProxyCacheServer getProxy(Context context, File file) {
        ...
        Log.i("old", ProxyCacheManager.instance().mCacheDir.getAbsolutePath());
