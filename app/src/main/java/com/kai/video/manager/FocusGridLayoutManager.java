...
public class FocusGridLayoutManager extends GridLayoutManager {
     ...
     */
    public FocusGridLayoutManager(Context context, int spanCount, VideoItemAdapter adapter) {
    ...
    public FocusGridLayoutManager(Context context, int spanCount, int orientation, boolean reverseLayout, RecyclerView recyclerView) {
    ...
    public View onInterceptFocusSearch(@NonNull View focused, int direction) {
        ...
        if (direction == View.FOCUS_DOWN || direction == View.FOCUS_UP) {
            ...
            currentPosition = adapter.getExactPosition(currentPosition, direction);
            //Log.i("tag", currentPosition + "cur");
            if (currentPosition < 0) currentPosition = 0;
            LogUtil.i("tag", "onInterceptFocusSearch: current position=" + currentPosition);
            LogUtil.i("tag", "onInterceptFocusSearch: item count=" + count);
            LogUtil.i("tag", "onInterceptFocusSearch: lastVisiblePosition=" + lastVisiblePosition);
            if (direction == View.FOCUS_DOWN) {
                scrolloffset(-20);
            }
            if (direction == View.FOCUS_UP && currentPosition < firstVisiblePosition){
                scrolloffset(20);
                if (adapter.getItemViewType(currentPosition) == 2){
                    LinearTopSmoothScroller smoothScroller = new LinearTopSmoothScroller(focused.getContext());
                    smoothScroller.setTargetPosition(currentPosition);
                    startSmoothScroll(smoothScroller);
                    final int futurePosition = currentPosition;
                    new Handler().postDelayed(() -> {
                        //Log.i("tag", "header");
                        findViewByPosition(futurePosition).findViewById(R.id.main).requestFocus();
                    }, 200);
                    return focused;
                }
            }
    ...
    public void setOnReachHeader(OnReachHeader onReachHeader) {
    }

    public interface OnReachHeader{
        void onReach(int position);
    }
}