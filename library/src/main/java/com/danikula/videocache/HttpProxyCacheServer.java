...
public class HttpProxyCacheServer {
    ...
    @SuppressLint("StaticFieldLeak")
    public static HttpProxyCacheServerClients ffconcatClients = null;

    private final Object clientsLock = new Object();
    private final ExecutorService socketProcessor = Executors.newFixedThreadPool(15);
    ...
    private void processSocket(Socket socket) {
        try {
            ...
            Log.e("Request to cache proxy" , url);
            if (pinger.isPingRequest(url)) {
                pinger.responseToPing(socket);
            } else {
                HttpProxyCacheServerClients clients;
                if (ProxyCacheUtils.isConcatSegment(url) && ffconcatClients != null){
                    clients = ffconcatClients;
                    ffconcatClients.setProxy(this);
                }
                    ...
                    if (ProxyCacheUtils.isConcatList(url)){
                        //发现concat列表文件请求，保存clients，并增加引用计数，让fragment请求
                        ffconcatClients = clients;
                        ffconcatClients.setProxy(this);
                        ffconcatClients.incrementAndGet();
                    }
                    else if (ProxyCacheUtils.isM3U8(url)) {
                        Log.e("proxy", "发现新m3u8清单");
    ...
    public static final class Builder {
         ...
         */
        public void cacheDirectory(File file) {
         ...
         */
        public void fileNameGenerator(FileNameGenerator fileNameGenerator) {
